    ...
    if ($id) {
        if ($password !== '') {
            $pdo->prepare('UPDATE users SET name=?, email=?, role=?, active=?, password_hash=? WHERE id=?')
                ->execute([$name, $email, $role, $active, password_hash($password, PASSWORD_DEFAULT), $id]);
        } else {
            $pdo->prepare('UPDATE users SET name=?, email=?, role=?, active=? WHERE id=?')
                ->execute([$name, $email, $role, $active, $id]);
        }
        audit_log('update_user', 'user', $id, ['name' => $name, 'email' => $email, 'role' => $role, 'active' => $active]);
        json_response(['success' => true, 'id' => $id]);
    } else {
        $pdo->prepare('INSERT INTO users (name, email, password_hash, role, active) VALUES (?, ?, ?, ?, ?)')
            ->execute([$name, $email, password_hash($password, PASSWORD_DEFAULT), $role, $active]);
        $newId = (int) $pdo->lastInsertId();
        audit_log('create_user', 'user', $newId, ['name' => $name, 'email' => $email, 'role' => $role]);
